...
const SignUp = () => {
	...
	const { signUpWithEmail, loading, error, user } = useAuthStore()
	...
	const handleSubmit = async (event) => {
		...
		if (!formState.email.trim() || !formState.password.trim() || !formState.name.trim()) {
			return
		}
		await signUpWithEmail({ name: formState.name, email: formState.email, password: formState.password })
	}
		...
		<div className="min-h-screen bg-gradient-to-br from-white via-purple-50 to-white px-6 py-16">
			<div className="mx-auto flex max-w-5xl flex-col gap-12 lg:flex-row">
				<div className="flex-1 space-y-6">
					...
					<div>
						<p className="text-xs uppercase tracking-[0.4em] text-purple-400">Create account</p>
						<h1 className="mt-3 text-4xl font-semibold text-slate-900">Join StrataGem</h1>
						<p className="mt-3 text-base text-slate-600">Centralize meetings, rituals, and action items in one collaborative operating system.</p>
					</div>
					<div className="rounded-3xl border border-purple-100 bg-white/80 p-6 shadow-xl backdrop-blur">
						<p className="text-sm font-semibold text-slate-700">Already have an account?</p>
						<p className="text-sm text-slate-500">Head to the sign-in page.</p>
						<Link to="/signin" className="mt-4 inline-flex items-center justify-center rounded-full border border-purple-200 px-5 py-2 text-sm font-semibold text-purple-700 transition hover:border-purple-400">Sign in</Link>
					</div>
				</div>
